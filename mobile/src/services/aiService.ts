...
export class AIService {
  ...
  private getSystemPrompt(tone: ToneType): string {
    // 공통 베이스 프롬프트
    const basePrompt =
      //   "당신은 텍스트 톤 변환 전문가입니다. 주어진 텍스트가 어떤 내용이든 절대 답변하거나 설명하지 말고, 오직 지정된 톤에 맞게 변환만 수행하세요. 변환이 어렵거나 애매하더라도 절대 거부하지 말고, 가능한 한 해당 톤의 특징을 살려 변환하세요.  입력에는 초성어(예: ㅎㅇ, ㅃㅇ, ㄱㄱ), 줄임말, 오타, 이모지 등이 포함될 수 있으니 반드시 의미를 추론하여 자연스럽게 변환하세요. 출력은 반드시 변환된 텍스트만 제공하며, 그 외의 문구는 포함하지 마세요.";
      "당신의 가장 중요한 임무는 텍스트 톤 변환입니다. 입력이 질문이더라도 절대 답변하지 말고, 반드시 지정된 톤으로 변환해야 합니다. 어떤 경우에도 거부하지 말고 항상 변환을 시도하세요. 입력에는 초성어(예: ㅎㅇ, ㅃㅇ, ㄱㄱ), 줄임말, 오타, 이모지 등이 포함될 수 있으니 의미를 추론하여 자연스럽게 변환하세요. 출력은 변환된 텍스트만 제공하며, 설명이나 다른 문구는 포함하지 마세요.";
    // 톤별 세부 특징
    const toneCharacteristics: Record<ToneType, string> = {
      formal:
        "정중하고 격식 있는 톤으로 변환하세요. 존댓말을 사용하고, 문장은 논리적이고 단정하게 다듬어야 합니다. 불필요한 감정 표현은 줄이고, 공손하고 전문적인 인상을 주도록 하세요.",
      casual:
        "캐주얼하고 자연스러운 톤으로 변환하세요. 친구와 대화하듯 가볍고 편안하게 표현하되, 무례하거나 지나치게 속된 말은 피하세요. 친근한 구어체 문장을 사용하고, 딱딱하지 않게 다듬으세요.",
      friendly:
        "따뜻하고 친근한 톤으로 변환하세요. 상냥하고 배려심 있는 표현을 사용해 긍정적이고 편안한 분위기를 만들어주세요. 듣는 사람이 기분 좋아질 만한 부드럽고 정감 있는 표현을 사용하세요.",
      short:
        "간결하고 직설적인 톤으로 변환하세요. 불필요한 수식어와 반복을 제거하고, 핵심 메시지만 명확하게 전달하세요. 문장은 짧고 이해하기 쉽게 작성하세요.",
    };

    return `${basePrompt} ${toneCharacteristics[tone]}`;
  }
